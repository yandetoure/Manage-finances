        ...
        <div class="glass-card">
            <form action="{{ route('revenues.update', $revenue->id) }}" method="POST">
                    ...
                    <div
                        ...
                        @foreach($categories as $category)
                            <label style="flex-shrink: 0; cursor: pointer;">
                                <input type="radio" name="category_id" value="{{ $category->id }}" required {{ $revenue->category_id == $category->id ? 'checked' : '' }} style="display: none;"
                ...
                <div style="margin-bottom: 20px;">
                    ...
                    <div style="position: relative;">
                        <input type="text" name="source" value="{{ $revenue->source }}" required
                            placeholder="Ex: Salaire, Freelance..." class="input-modern"
                ...
                <div style="margin-bottom: 20px;">
                    ...
                    <div style="position: relative;">
                        <input type="number" step="0.01" name="amount" value="{{ $revenue->amount }}" required
                ...
                <div style="margin-bottom: 20px;">
                    ...
                    <div style="position: relative;">
                        <input type="date" name="due_date"
                            value="{{ $revenue->due_date ? \Carbon\Carbon::parse($revenue->due_date)->format('Y-m-d') : '' }}"
                            class="input-modern"
                            style="width: 100%; background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1); border-radius: 16px; padding: 14px 14px 14px 45px; color: white; outline: none; transition: all 0.3s;">
                        <div style="position: absolute; left: 15px; top: 50%; transform: translateY(-50%); opacity: 0.5;">
                            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none"
                ...
                <div
                    ...
                    <div id="frequency-container"
                        style="{{ $revenue->is_recurrent ? 'display: block;' : 'display: none;' }} margin-top: 15px; padding-top: 15px; border-top: 1px solid rgba(255,255,255,0.05);">
                        <label class="text-muted"
                        ...
                        <select name="frequency" class="input-modern"
                            ...
                            <option value="weekly" {{ $revenue->frequency == 'weekly' ? 'selected' : '' }}>Hebdomadaire
                            </option>
