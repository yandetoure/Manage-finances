                ...
                <div class="glass-card" @click="openMenu({{ Js::from($claim) }})"
                    ...
                    <div style="text-align: right;">
                        <p class="text-bold" style="color: #22c55e; font-size: 15px;">
                            {{ number_format($claim->amount, 0, ',', ' ') }} FCFA</p>
                        <span
                            style="font-size: 10px; background: {{ $claim->status == 'paid' ? 'rgba(16, 185, 129, 0.15)' : ($claim->status == 'late' ? 'rgba(239, 68, 68, 0.15)' : 'rgba(245, 158, 11, 0.15)') }}; color: {{ $claim->status == 'paid' ? '#10b981' : ($claim->status == 'late' ? '#ef4444' : '#f59e0b') }}; padding: 3px 8px; border-radius: 8px; font-weight: 600;">
