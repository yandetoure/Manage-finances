...
@section('content')
    <div class="fade-in">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 25px;">
            <h2 class="text-bold">{{ __('Ma Vision Globale') }}</h2>
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px; margin-bottom: 25px;">
            <div class="glass-card" style="padding: 20px;">
                <p class="text-muted"
                    style="font-size: 11px; text-transform: uppercase; font-weight: 600; margin-bottom: 10px;">
                    {{ __('Revenus') }}</p>
            ...
            <div class="glass-card" style="padding: 20px;">
                ...
                <h3 class="text-bold" style="font-size: 20px; color: #ef4444;">
                    {{ number_format($totalExpenses, 0, ',', ' ') }}</h3>
            </div>
            <div class="glass-card" style="padding: 20px;">
                <p class="text-muted"
                    style="font-size: 11px; text-transform: uppercase; font-weight: 600; margin-bottom: 10px;">
                ...
                <h3 class="text-bold" style="font-size: 20px; color: var(--accent-blue);">
                    {{ number_format($totalSavings, 0, ',', ' ') }}</h3>
            </div>
            <div class="glass-card" style="padding: 20px;">
                <p class="text-muted"
                    style="font-size: 11px; text-transform: uppercase; font-weight: 600; margin-bottom: 10px;">
                    {{ __('Dettes Créées') }}</p>
                <h3 class="text-bold" style="font-size: 20px; color: #f59e0b;">{{ number_format($totalDebts, 0, ',', ' ') }}
                </h3>
            </div>
        </div>
        ...
        <div class="glass-card" style="padding: 20px; margin-bottom: 30px;">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
                <h3 class="text-bold" style="font-size: 16px;">{{ __('Résumé de la période') }}</h3>
                <span class="text-muted"
                    style="font-size: 11px;">{{ Carbon\Carbon::create()->month($month)->translatedFormat('F') }}
                    {{ $year }}</span>
            </div>
    ...
    <div class="glass-card" style="padding: 20px; margin-bottom: 25px;">
        <h3 class="text-bold" style="font-size: 16px; margin-bottom: 20px;">{{ __('Répartition des Dépenses') }}</h3>
        ...
        <div id="categoryLegend" style="margin-top: 20px; display: grid; grid-template-columns: 1fr 1fr; gap: 10px;">
            @foreach($expensesByCategory as $cat)
                <div style="display: flex; align-items: center; gap: 8px;">
                    ...
                    <span
                        style="font-size: 11px; font-weight: 700; margin-left: auto;">{{ number_format($cat['amount'], 0, ',', ' ') }}</span>
                </div>
            @endforeach
        </div>
    </div>
    ...
    <div class="glass-card" style="padding: 20px; margin-bottom: 25px;">
        <h3 class="text-bold" style="font-size: 16px; margin-bottom: 20px;">{{ __('Évolution (6 mois)') }}</h3>
    ...
    <div class="glass-card" style="padding: 20px; margin-bottom: 30px;">
        ...
        <div style="display: flex; flex-direction: column; gap: 12px;">
            <div
                style="display: flex; justify-content: space-between; padding: 10px 0; border-bottom: 1px solid var(--card-border);">
                <span class="text-muted" style="font-size: 13px;">{{ __('Solde Net (Revenus - Dépenses)') }}</span>
                <span class="text-bold {{ $balance >= 0 ? 'text-green' : 'text-bold' }}"
                    style="font-size: 13px; color: {{ $balance >= 0 ? 'var(--primary-green)' : '#ef4444' }};">
                    {{ $balance >= 0 ? '+' : '' }} {{ number_format($balance, 0, ',', ' ') }} {{ auth()->user()->currency }}
                </span>
            </div>
            <div
                style="display: flex; justify-content: space-between; padding: 10px 0; border-bottom: 1px solid var(--card-border);">
                <span class="text-muted" style="font-size: 13px;">{{ __("Taux d'épargne") }}</span>
        ...
        <a href="{{ route('home') }}" class="btn btn-primary"
            style="width: 100%; margin-top: 20px; text-align: center; border-radius: 18px; padding: 16px; font-size: 15px; font-weight: 700;">{{ __('Retour au Dashboard') }}</a>
    </div>
    </div>
@endsection