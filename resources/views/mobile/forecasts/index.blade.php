...
@section('content')
    <div class="fade-in">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 25px;">
            <h2 class="text-bold">{{ __('Prévisions') }}</h2>
            <a href="{{ route('forecasts.create') }}" class="btn btn-accent">+ {{ __('Prévoir') }}</a>
        </div>

        @if($forecasts->isEmpty())
            <div class="glass-card" style="text-align: center; padding: 40px 20px;">
                <p class="text-muted">{{ __('Aucune prévision pour le moment.') }}</p>
            </div>
        @else
                ...
                @foreach($forecasts as $forecast)
                    <div class="glass-card" style="padding: 20px;">
                        ...
                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
                            <div>
                                <p class="text-muted" style="font-size: 11px;">{{ __('Revenus Est.') }}</p>
                                <p class="text-bold" style="color: var(--accent-blue);">
                                    +{{ number_format($forecast->estimated_revenue, 0, ',', ' ') }}</p>
                            </div>
                            <div>
                                <p class="text-muted" style="font-size: 11px;">{{ __('Dépenses Est.') }}</p>
                        ...
                        <div style="display: flex; justify-content: space-between; align-items: center;">
                            <span class="text-muted" style="font-size: 12px;">{{ __('Solde Projeté') }}</span>
